...
description = "Independent BlendLib ecosystem example using published public coordinates only."

base {
    archivesName.set("blendlib-ecosystem-example")
}
...
tasks.withType<Jar>().configureEach {
    manifest {
        attributes(mapOf(
            "BlendLib-Artifact" to "ecosystem-example",
...
loom {
    ...
    mods {
        create("blendlib_ecosystem_example") {
    ...
    runs {
        named("client") {
            setConfigName("BlendLib Ecosystem Example Client")
            runDir("run/client")
        }
        named("server") {
            setConfigName("BlendLib Ecosystem Example Server")
            runDir("run/server")
        }
    }
}
